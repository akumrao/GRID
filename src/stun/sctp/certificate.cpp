...
Certificate_good Certificate_good::FromString(string crt_pem, string key_pem) {
	SDebug << "Importing certificate from PEM string (GnuTLS)";
	...
	return Certificate_good(std::move(creds));
}

Certificate_good Certificate_good::FromFile(const string &crt_pem_file, const string &key_pem_file,
                                  const string &pass) {
	SDebug << "Importing certificate from PEM file (GnuTLS): " << crt_pem_file;
...
Certificate_good Certificate_good::Generate(CertificateType type, const string &commonName) {
	SDebug << "Generating certificate (GnuTLS)";
	...
	return Certificate_good(*crt, *privkey);
}

Certificate_good::Certificate_good(gnutls_x509_crt_t crt, gnutls_x509_privkey_t privkey)
...
Certificate_good::Certificate_good(shared_ptr<gnutls_certificate_credentials_t> creds)
    : mCredentials(std::move(creds)),
      mFingerprint(make_fingerprint(*mCredentials, CertificateFingerprint::Algorithm::Sha256)) {}

gnutls_certificate_credentials_t Certificate_good::credentials() const { return *mCredentials; }
...
Certificate_good::Certificate_good(shared_ptr<mbedtls_x509_crt> crt, shared_ptr<mbedtls_pk_context> pk)
    : mCrt(crt), mPk(pk),
...
Certificate_good Certificate_good::FromString(string crt_pem, string key_pem) {
	...
	return Certificate_good(std::move(crt), std::move(pk));
}
...
Certificate_good Certificate_good::Generate(CertificateType type, const string &commonName) {
	...
	try {
		...
		if (certificateLen <= 0) {
			throw std::runtime_error("Certificate_good generation failed");
		}
...
Certificate_good::credentials() const {
	return {mCrt, mPk};
}
...
Certificate_good Certificate_good::FromString(string crt_pem, string key_pem) {
	...
	return Certificate_good(x509, pkey, std::move(chain));
}
...
Certificate_good Certificate_good::Generate(CertificateType type, const string &commonName) {
	...
	return Certificate_good(x509, pkey);
}

Certificate_good::Certificate_good(shared_ptr<X509> x509, shared_ptr<EVP_PKEY> pkey,
                         std::vector<shared_ptr<X509>> chain)
...
std::tuple<X509 *, EVP_PKEY *> Certificate_good::credentials() const {
	return {mX509.get(), mPKey.get()};
}

std::vector<X509 *> Certificate_good::chain() const {
	std::vector<X509 *> v;
...
string make_fingerprint_good(X509 *x509, CertificateFingerprint::Algorithm fingerprintAlgorithm) {
...
future_certificate_ptr make_certificate_good(CertificateType type) {
	return ThreadPool::Instance().enqueue([type, token = Init::Instance().token()]() {
		return std::make_shared<Certificate_good>(Certificate_good::Generate(type, "libdatachannel"));
	});
}

CertificateFingerprint Certificate_good::fingerprint() const {
