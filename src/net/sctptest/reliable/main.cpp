...
shared_ptr<Client> createPeerConnection(Configuration &config, string id, bool isClient) {
...

    {
        ...
        auto dc = pc->createDataChannel(dcchat, dcInit);
        dc->onOpen([id, wdc = make_weak_ptr(dc)](){
            if (auto dc = wdc.lock()) {
                ...
                std::cout << "[SENDER] Unreliable channel OPEN (maxRetransmits=3)" << std::endl;

                        // Send a burst of sequential packets to test the loss threshold
                for (int seq = 1; seq <= 50; ++seq) {
                    std::string payload = std::to_string(seq) + ":TestDataPacket";

                            std::cout << "[SENDER] Sending packet -> SEQ: " << seq << std::endl;
                            dc->send(payload);
                }
        ...
        int expectedSeq = 1;
        dc->onMessage([id, dc, &expectedSeq ](auto data) {
            // data holds either std::string or rtc::binary
            if (std::holds_alternative<std::string>(data)) {
                std::string msg = std::get<std::string>(data);

                // Parse sequence number from payload (Format: "seq:content")
                size_t delimiter = msg.find(':');
                if (delimiter != std::string::npos) {
                    int receivedSeq = std::stoi(msg.substr(0, delimiter));

                    // If received sequence is higher than expected, gap/loss occurred
                    if (receivedSeq > expectedSeq) {
                        std::cout << "[RECEIVER] !! LOSS DETECTED !! Missed SEQ range: ["
                                << expectedSeq << " to " << (receivedSeq - 1) << "]" << std::endl;
                    }

                    std::cout << "[RECEIVER] Successfully received -> SEQ: " << receivedSeq << std::endl;

                    // Sync the counter to expect the next item sequentially
