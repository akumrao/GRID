...
namespace stun {
  ...
  MessageStun::MessageStun(uint16_t type)
    :type(type)
    ,length(0)
    ,cookie(0x2112a442)
  {
  ...
  MessageStun::MessageStun(stun_class_t msg_class, stun_method_t msg_method)
  ...
  void MessageStun::copyTransactionID(MessageStun* from) {

    if (!from) {
      printf("Error: tryign to copy a transaction ID, but the message is invalid in stun::MessageStun.\n");
      return;
    }
...
//    transaction[2] = from->transaction[2];
  ...
  void MessageStun::setTransactionID() {
//    transaction[0] = a;
//    transaction[1] = b;
//    transaction[2] = c;
    ...
    printf("MessageStun::Set msg  transactionids: ");
   ...
   void MessageStun::setTransactionID( uint8_t *transactionid) {
        memcpy(transaction_id, transactionid, STUN_TRANSACTION_ID_SIZE);
   }

  
  bool MessageStun::find(MessageIntegrity** result) {
  ...
  bool MessageStun::find(XorMappedAddress** result) {
    return find<XorMappedAddress>(STUN_ATTR_XOR_MAPPED_ADDRESS, result);
  }

  bool MessageStun::find(Fingerprint** result) {
    return find<Fingerprint>(STUN_ATTR_FINGERPRINT, result);
  }
  
  bool MessageStun::find(Priority** result) {
    return find<Priority>(STUN_ATTR_PRIORITY, result);
  }

  bool MessageStun::hasAttribute(AttributeType atype) {
