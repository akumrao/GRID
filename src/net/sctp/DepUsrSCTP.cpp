...
static constexpr size_t CheckerInterval
{
    10u
}; // In ms.
...
inline static int onSendSctpData(void* addr, void* data, size_t len, uint8_t /*tos*/, uint8_t /*setDf*/) {
    auto* sctpAssociation = static_cast<rtc::SctpTransport*> (addr);
...
uint64_t DepUsrSCTP::numSctpTransports{ 0u};
...
void DepUsrSCTP::IncreaseSctpTransports() {


    if (++DepUsrSCTP::numSctpTransports == 1u)
        DepUsrSCTP::checker->Start();
}

void DepUsrSCTP::DecreaseSctpTransports() {


    assert(DepUsrSCTP::numSctpTransports > 0u);

    if (--DepUsrSCTP::numSctpTransports == 0u)
...
void DepUsrSCTP::Checker::Start() {
    ...
    SDebug << "usrsctp periodic check started";
...
void DepUsrSCTP::Checker::Stop() {
    ...
    SDebug << "usrsctp periodic check stopped";
