...
namespace rtc {
    ...
    TransportTuple* IceServer::AddTuple(TransportTuple* tuple) {
        ...
        TransportTuple* ret = HasTuple(tuple);
...
//            return this->selectedTuple;
    ...
    // Add this at the bottom of IceServer.cpp inside the rtc namespace block

    // =========================================================================
    // TCP Connection Management
    // =========================================================================

    TcpConnectionBase* IceServer::find_tcp_connection(const addr_record_t& record) {
        std::lock_guard<std::mutex> lock(m_connectionMutex);

        auto it = m_tcpConnections.find(record);
        if (it != m_tcpConnections.end()) {
            TcpConnectionBase* conn = it->second;
            if (conn) {
                return conn;
            }
        }
    ...
    void IceServer::register_tcp_connection(const addr_record_t& record, TcpConnectionBase* conn) {
        if (!conn) return;
        std::lock_guard<std::mutex> lock(m_connectionMutex);

        m_tcpConnections[record] = conn;
    }

    void IceServer::unregister_tcp_connection(const addr_record_t& record) {
        std::lock_guard<std::mutex> lock(m_connectionMutex);

        m_tcpConnections.erase(record);
    }

    void IceServer::clear_connections() {
        std::lock_guard<std::mutex> lock(m_connectionMutex);

        // 1. Cleanup TCP Connections
        for (auto& pair : m_tcpConnections) {
            if (pair.second) {
                pair.second->Close(); // Triggers the socket shutdown sequence
                delete pair.second; // Deletes the dynamically allocated pointer
            }
        }
