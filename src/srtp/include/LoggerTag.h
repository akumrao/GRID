...
//#define 1) Settings::configuration.logTags.tag
...
#define MS_DEBUG_2TAGS( ...) { LogStream(Level::Trace, _fileName(__FILE__), __LINE__).write(__VA_ARGS__); }

#define MS_WARN_2TAGS( ...) { LogStream(Level::Trace, _fileName(__FILE__), __LINE__).write(__VA_ARGS__); }
...
#define MS_DEBUG_TAG(tag, ...) { LogStream(Level::Trace, _fileName(__FILE__), __LINE__).write(__VA_ARGS__); }
