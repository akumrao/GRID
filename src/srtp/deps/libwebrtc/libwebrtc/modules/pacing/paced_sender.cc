...
PacedSender::PacedSender(PacketRouter* packet_router,
    ...
    : packet_router_(packet_router),
      ...
      time_last_process_us_(base::Application::GetTimeUs()),
...
void PacedSender::CreateProbeCluster(int bitrate_bps, int cluster_id) {
  ...
  prober_.CreateProbeCluster(bitrate_bps, base::Application::GetTimeMs(), cluster_id);
}
...
void PacedSender::SetProbingEnabled(bool enabled) {
  ...
  assertm(packet_counter_ == 0, "packet counter must be 0");
...
void PacedSender::SetPacingRates(uint32_t pacing_rate_bps,
  ...
  assertm(pacing_rate_bps > 0, "pacing rate must be > 0");
...
void PacedSender::InsertPacket(size_t bytes) {
  ...
  assertm(pacing_bitrate_kbps_ > 0, "SetPacingRates() must be called before InsertPacket()");
...
int64_t PacedSender::TimeUntilNextProcess() {
  int64_t elapsed_time_us =
      base::Application::GetTimeUs() - time_last_process_us_;
  ...
  if (prober_.IsProbing()) {
    int64_t ret = prober_.TimeUntilNextProbe(base::Application::GetTimeMs());
...
void PacedSender::Process() {
  int64_t now_us = base::Application::GetTimeUs();
  ...
  if (bytes_sent != 0)
  {
    auto now = base::Application::GetTimeUs();
...
size_t PacedSender::PaddingBytesToAdd(
  ...
  // MS_NOTE: This does not apply to sfuserver.
...
void PacedSender::OnPacketSent(size_t size) {
  if (first_sent_packet_ms_ == -1)
    first_sent_packet_ms_ = base::Application::GetTimeMs();
