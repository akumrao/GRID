...
TargetRateConstraints ConvertConstraints(int min_bitrate_bps,
  ...
  msg.at_time = Timestamp::ms(base::Application::GetTimeMs());
  msg.min_data_rate =
...
RtpTransportControllerSend::RtpTransportControllerSend(
    ...
    : packet_router_(packet_router),
      ...
      last_report_block_time_(Timestamp::ms(base::Application::GetTimeMs())),
  ...
  assertm(bitrate_config.start_bitrate_bps > 0, "start bitrate must be > 0");
...
void RtpTransportControllerSend::UpdateControlState() {
  ...
  assertm(observer_ != nullptr, "no observer");
...
void RtpTransportControllerSend::RegisterTargetTransferRateObserver(
    ...
    // RTC_DCHECK(observer_ == nullptr);
    assertm(observer_ == nullptr, "observer already set");
...
void RtpTransportControllerSend::OnReceivedEstimatedBitrate(uint32_t bitrate) {
  ...
  msg.receive_time = Timestamp::ms(base::Application::GetTimeMs());
...
void RtpTransportControllerSend::OnAddPacket(
  ...
  transport_feedback_adapter_.AddPacket(
      ...
      send_side_bwe_with_overhead_ ? transport_overhead_bytes_per_packet_.load()
                                   : 0,
      Timestamp::ms(base::Application::GetTimeMs()));
}
...
void RtpTransportControllerSend::OnTransportFeedback(
  ...
  absl::optional<TransportPacketsFeedback> feedback_msg =
      transport_feedback_adapter_.ProcessTransportFeedback(
          feedback, Timestamp::ms(base::Application::GetTimeMs()));
  if (feedback_msg)
...
void RtpTransportControllerSend::OnRemoteNetworkEstimate(
  ...
  estimate.update_time = Timestamp::ms(base::Application::GetTimeMs());
...
void RtpTransportControllerSend::Process()
{
  ...
  // https://bitbucket.org/versatica/sfuserver/issues/12/no-probation-if-no-real-media
...
void RtpTransportControllerSend::MaybeCreateControllers() {
  ...
  assertm(!controller_, "controller already set");
  assertm(!control_handler_, "controller handler already set");
...
void RtpTransportControllerSend::UpdateControllerWithTimeInterval() {
  // RTC_DCHECK(controller_);
  assertm(controller_, "controller not set");
...
void RtpTransportControllerSend::UpdateStreamsConfig() {
  streams_config_.at_time = Timestamp::ms(base::Application::GetTimeMs());
  if (controller_)
