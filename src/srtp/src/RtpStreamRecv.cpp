...
namespace rtc
{
	  ...
	  uint64_t nowMs, uint8_t spatialLayer, uint8_t temporalLayer)
	{
		...
		assertm(spatialLayer < this->spatialLayerCounters.size(), "spatialLayer too high");
		assertm(
	...
	bool RtpStreamRecv::ReceiveRtxPacket(rtc::RtpPacket* packet)
	{
		...
		if (!this->params.useNack)
		{
			MS_WARN_TAG( "NACK not supported");

			return false;
		}

		assertm(packet->GetSsrc() == this->params.rtxSsrc, "invalid ssrc on RTX packet");
		...
		if (HasRtx())
		{
			...
			{
				MS_WARN_TAG( "RTX packet discarded");
	...
	rtc::RTCP::ReceiverReport* RtpStreamRecv::GetRtcpReceiverReport()
	{
		...
		if (this->lastSrReceived != 0)
		{
			...
			auto delayMs = static_cast<uint32_t>(base::Application::GetTimeMs() - this->lastSrReceived);
	...
	void RtpStreamRecv::ReceiveRtcpSenderReport(rtc::RTCP::SenderReport* report)
	{
		...
		this->lastSrReceived  = base::Application::GetTimeMs();
	...
	void RtpStreamRecv::RequestKeyFrame()
	{
		...
		{
			MS_DEBUG_2TAGS( "sending PLI [ssrc:%" PRIu32 "]", GetSsrc());
		...
		{
			MS_WARN_TAG( "sending FIR [ssrc:%" PRIu32 "]", GetSsrc());
	...
	void RtpStreamRecv::CalculateJitter(uint32_t rtpTimestamp)
	{
		...
		auto transit =
		  static_cast<int>(base::Application::GetTimeMs() - (rtpTimestamp * 1000 / this->params.clockRate));
	...
	void RtpStreamRecv::UpdateScore()
	{
		...
		assertm(retransmitted >= repaired, "repaired packets cannot be more than retransmitted ones");
	...
	inline void RtpStreamRecv::OnTimer(Timer* timer)
	{
		...
		if (timer == this->inactivityCheckPeriodicTimer)
		{
			...
			{
				MS_WARN_2TAGS(
				    "RTP inactivity detected, resetting score to 0 [ssrc:%" PRIu32 "]", GetSsrc());
			}
	...
	inline void RtpStreamRecv::OnNackGeneratorNackRequired(const std::vector<uint16_t>& seqNumbers)
	{
		...
		assertm(this->params.useNack, "NACK required but not supported");
