...
bool IsRtcp(const binary &data) {
	...
	std::cout << "Demultiplexing RTCP and RTP with payload type, value=" << int(payloadType);
...
void RtpHeader::log() const {
	std::cout << "RtpHeader V: " << (int)version() << " P: " << (padding() ? "P" : " ")
