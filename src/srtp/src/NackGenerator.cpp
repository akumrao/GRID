...
namespace RTC
{
	...
	void NackGenerator::AddPacketsToNackList(uint16_t seqStart, uint16_t seqEnd)
	{
		...
		if (this->nackList.size() + numNewNacks > MaxNackPackets)
		{
			...
			{
				MS_WARN_TAG(
				   "NACK list full, clearing it and requesting a key frame [seqEnd:%" PRIu16 "]", seqEnd);
		...
		{
			assertm(this->nackList.find(seq) == this->nackList.end(), "packet already in the NACK list");
