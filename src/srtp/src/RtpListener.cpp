	...
	void RtpListener::AddProducer(RTC::Producer* producer)
	{
		...
		for (auto& encoding : rtpParameters.encodings)
		{
			...
			if (ssrc != 0u)
			{
				...
				else
				{
					...
					base::uv::throwError("ssrc already exists in RTP listener [ssrc:%" PRIu32 "]", ssrc);
				}
			}
			...
			if (ssrc != 0u)
			{
				...
				else
				{
					...
					base::uv::throwError("RTX ssrc already exists in RTP listener [ssrc:%" PRIu32 "]", ssrc);
				}
			}
		}
		...
		if (!rtpParameters.mid.empty())
		{
			...
			else
			{
				...
				base::uv::throwError("MID already exists in RTP listener [mid:] " + mid);
			}
		}
		...
		for (auto& encoding : rtpParameters.encodings)
		{
			...
			else if (rtpParameters.mid.empty())
			{
				...
				base::uv::throwError("RID already exists in RTP listener and no MID is given [rid:] " + rid);
			}
		}
	}
