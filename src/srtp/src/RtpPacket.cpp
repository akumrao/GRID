...
namespace rtc
{
	...
	RtpPacket* RtpPacket::Parse(const uint8_t* data, size_t len)
	{
		...
		if (header->csrcCount != 0u)
		{
			...
			{
				MS_WARN_TAG("not enough space for the announced CSRC list, packet discarded");
		...
		if (header->extension == 1u)
		{
			...
			{
				MS_WARN_TAG("not enough space for the announced header extension, packet discarded");
			...
			{
				MS_WARN_TAG( "not enough space for the announced header extension value, packet discarded");
		...
		if (header->padding != 0u)
		{
			...
			if (payloadLength == 0)
			{
				MS_WARN_TAG("padding bit is set but no space for a padding byte, packet discarded");
			...
			if (payloadPadding == 0)
			{
				MS_WARN_TAG("padding byte cannot be 0, packet discarded");
			...
			{
				MS_WARN_TAG(  "number of padding octets is greater than available space for payload, packet "
				  "discarded");
		...
		if (HasOneByteExtensions())
		{
			...
			while (ptr < extensionEnd)
			{
				...
				if (id != 0u)
				{
					...
					{
						MS_WARN_TAG( "not enough space for the announced One-Byte header extension element value");
		...
		else if (HasTwoBytesExtensions())
		{
			...
			while (ptr + 1 < extensionEnd)
			{
				...
				if (id != 0u)
				{
					...
					{
						MS_WARN_TAG(
						   "not enough space for the announced Two-Bytes header extension element value");
