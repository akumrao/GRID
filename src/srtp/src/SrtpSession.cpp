...
namespace rtc
{
	...
	{       
		...
		{
			base::uv::throwError("srtp_install_event_handler() failed: " + std::string(DepLibSRTP::GetErrorString(err)));
		}
	}
	...
	void SrtpSession::OnSrtpEvent(srtp_event_data_t* data)
	{
		...
		switch (data->event)
		{
			case event_ssrc_collision:
				MS_WARN_TAG( "SSRC collision occurred");
				break;

			case event_key_soft_limit:
				MS_WARN_TAG( "stream reached the soft key usage limit and will expire soon");
				break;

			case event_key_hard_limit:
				MS_WARN_TAG( "stream reached the hard key usage limit and has expired");
				break;

			case event_packet_index_limit:
				MS_WARN_TAG( "stream reached the hard packet limit (2^48 packets)");
				break;
		}
	}
	...
	SrtpSession::SrtpSession(Type type, Profile profile, uint8_t* key, size_t keyLen)
	{
		...
		if (DepLibSRTP::IsError(err))
			base::uv::throwError("srtp_create() failed: " + std::string(DepLibSRTP::GetErrorString(err)));
	}
	...
	bool SrtpSession::EncryptRtp(const uint8_t** data, size_t* len)
	{
		...
		{
			MS_WARN_TAG( "cannot encrypt RTP packet, size too big (%zu bytes)", *len);
		...
		if (DepLibSRTP::IsError(err))
		{
			MS_WARN_TAG( "srtp_protect() failed: %s", DepLibSRTP::GetErrorString(err));
	...
	bool SrtpSession::EncryptRtcp(const uint8_t** data, size_t* len)
	{
		...
		if (*len + SRTP_MAX_TRAILER_LEN > EncryptBufferSize)
		{
			MS_WARN_TAG( "cannot encrypt RTCP packet, size too big bytes ", *len);
		...
		if (DepLibSRTP::IsError(err))
		{
			MS_WARN_TAG( "srtp_protect_rtcp() failed: ", DepLibSRTP::GetErrorString(err));
	...
	bool SrtpSession::DecryptSrtcp(uint8_t* data, size_t* len)
	{
		...
		if (DepLibSRTP::IsError(err))
		{
			MS_DEBUG_TAG(srtp, "srtp_unprotect_rtcp() failed:", DepLibSRTP::GetErrorString(err));
