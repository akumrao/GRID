...
namespace RTC
{
	namespace RTCP
	{
		  ...
		  : FeedbackRtpPacket(commonHeader)
		{
			...
			if (len > availableLen)
			{
				MS_WARN_TAG( "packet announced length exceeds the available buffer length, discarded");
			...
			while (count < this->packetStatusCount && contentLen > offset)
			{
				...
				{
					MS_WARN_TAG( "not enough space for chunk");
			...
			{
				MS_WARN_TAG( "provided packet status count does not match with content");
			...
			while (chunksIt != this->chunks.end() && contentLen > offset)
			{
				...
				{
					MS_WARN_TAG( "not enough space for deltas");
			...
			{
				MS_WARN_TAG( "received deltas does not match received status count");
		...
		void FeedbackRtpTransportPacket::AddPendingChunks()
		{
			...
			{
				assertm(this->context.statuses.size() < 7, "already 7 status packets present");
		  ...
		  const uint8_t* data, size_t len, uint16_t count)
		{
			...
			if (len < 2u)
			{
				MS_WARN_TAG( "not enough space for FeedbackRtpTransportPacket chunk, discarded");
		  ...
		  const uint8_t* data, size_t len, std::vector<int16_t>& deltas, size_t& offset)
		{
			...
			else if (this->status == Status::SmallDelta)
			{
				...
				{
					MS_WARN_TAG( "not enough space for small deltas");
			...
			else if (this->status == Status::LargeDelta)
			{
				...
				{
					MS_WARN_TAG( "not enough space for large deltas");
		...
		FeedbackRtpTransportPacket::OneBitVectorChunk::OneBitVectorChunk(uint16_t buffer, uint16_t count)
		{
			...
			assertm(buffer & 0x8000, "invalid one bit vector chunk");
		  ...
		  const uint8_t* data, size_t len, std::vector<int16_t>& deltas, size_t& offset)
		{
			...
			for (auto status : this->statuses)
			{
				...
				else if (status == Status::SmallDelta)
				{
					if (len < 1u)
					{
						MS_WARN_TAG( "not enough space for small delta");
				...
				{
					MS_WARN_TAG( "invalid status for one bit vector chunk");
		...
		size_t FeedbackRtpTransportPacket::OneBitVectorChunk::Serialize(uint8_t* buffer)
		{
			...
			assertm(this->statuses.size() <= 14, "packet info size must be 14 or less");
		...
		FeedbackRtpTransportPacket::TwoBitVectorChunk::TwoBitVectorChunk(uint16_t buffer, uint16_t count)
		{
			...
			assertm(buffer & 0xC000, "invalid two bit vector chunk");
		  ...
		  const uint8_t* data, size_t len, std::vector<int16_t>& deltas, size_t& offset)
		{
			...
			for (auto status : this->statuses)
			{
				...
				else if (status == Status::LargeDelta)
				{
					if (len < 2u)
					{
						MS_WARN_TAG( "not enough space for large delta");
		...
		size_t FeedbackRtpTransportPacket::TwoBitVectorChunk::Serialize(uint8_t* buffer)
		{
			...
			assertm(this->statuses.size() <= 7, "packet info size must be 7 or less");
