...
namespace rtc
{
	namespace RTCP
	{
		...
		FeedbackPsRpsiItem::FeedbackPsRpsiItem(Header* header)
		{
			...
			{
				MS_WARN_TAG( "invalid Rpsi packet with fractional padding bytes value");
			...
			{
				MS_WARN_TAG( "invalid Rpsi packet with too many padding bytes");
		...
		FeedbackPsRpsiItem::FeedbackPsRpsiItem(uint8_t payloadType, uint8_t* bitString, size_t length)
		{
			...
			assertm(payloadType <= 0x7f, "rpsi payload type exceeds the maximum value");
			assertm(
